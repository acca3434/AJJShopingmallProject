...
- 회원가입시 정규식을 작성하여 정규식 규칙 틀릴 시 회원가입 진행불가 기능구현.
...
- 유저가 마음에 드는 쇼핑몰의 물건을 누르면 장바구니에 담기는 기능.
...
- 메인 슬라이드 및 부가적인 슬라이드 기능 구현의 완료가 부실.

  - 담당했던 팀원 장지원이 부가적인 자료수집 및 해결.
  
- 유저가 풍선을 클릭하면 마일리지가 지급되는 구조에서 유저에게 지급이 되질 않음.

  - 디버깅 결과 해당 로직의 순서에 따라서 마일리지 지급 자체가 작동불가로 판단됌.
  
  - 파악 후 해결하도록 하겠음.
  
- 유저 베스트 검색 실시간 페이지 기능 고장

  - 원인은 상호작용하는 애니매이션이 때때로 거꾸로 넘어가는 애니매이션으로 작동됌.
  
  - 원인 파악 후 해결했음
  
- 데이터 베이스 설계 중 정규화를 따르지 않는데에서 생기는 처리 속도
